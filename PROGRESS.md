...
- ✅ **verify:** `node -c` 3 ไฟล์ OK / `bookDate()`=วันนี้ / `pool.status().bookDate`=วันนี้ / inline JS syntax OK / orphan เกลี้ยง
- ⏳ **รอ:** เทส live หน้ากากจริง (ช่วงเย็น DNP ปิดวันนี้ → ควรเห็นแบนเนอร์แดง + ปุ่มจองวันนี้ดับ) + เทสปุ่มจองพรุ่งนี้
