...
// วันเป้าหมาย = bookDate() = วันนี้เสมอ — จองวันอื่น (เช่นพรุ่งนี้) = warm สดทีละครั้ง (cold)
...
let targetDate = bookDate(); // วันเป้าหมาย = วันนี้ (ไม่เด้งพรุ่งนี้เอง)
...
async function warmOne() {
  warming++;
  try {
    const date = bookDate(); // วันเป้าหมาย = วันนี้เสมอ (ถ้า DNP ปิดทุกรอบ หน้ากากให้ จนท.กดจองพรุ่งนี้เอง)
    const tab = await warmTab(date, { log: logFn });
    targetDate = date;
    slots = tab.timeSlots;
    pool.push(tab);
    lastError = null;
...
async function acquire(date, log = console.log) {
  ...
  // ทิ้งแท็บที่ไม่ตรงวันเป้าหมาย (เช่น warm ค้างจากเมื่อวาน) — กันหยิบแท็บผิดวันมากรอก
  pool = pool.filter((t) => {
